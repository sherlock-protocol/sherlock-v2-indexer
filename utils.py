...
def get_premiums_apy(tvl, premiums):
    """
...
@retry(count=5)
def get_event_logs_in_range(session, event: Type[ContractEvent], start_block: int, end_block: int):
    ...
    resp = r.get(f"{EVENTS_CACHE_URL}/events?start={start_block}&end={end_block}")
    resp.raise_for_status()
    data = resp.json()

    # Filter only events that we need
    data = [x for x in data if x["event_name"] == event.__name__]
