...
DB = create_engine(
    "postgresql+psycopg2://{}:{}@localhost:{}/{}".format(
...
with open(
    os.path.join(REPO, "artifacts", "contracts", "Sherlock.sol", "Sherlock.json")
) as json_data:
...
with open(
    os.path.join(REPO, "artifacts", "contracts", "SherBuy.sol", "SherBuy.json")
) as json_data:
...
with open(
    os.path.join(REPO, "artifacts", "contracts", "SherClaim.sol", "SherClaim.json")
) as json_data:
    SHER_CLAIM_ABI = json.load(json_data)["abi"]
SHER_CLAIM_WSS = WEB3_WSS.eth.contract(address=SHER_CLAIM_ADDRESS, abi=SHER_CLAIM_ABI)

SHER_CLAIM_AT = (
    SHER_CLAIM_WSS.functions.newEntryDeadline().call() + 60 * 60 * 24 * 7 * 26
)  # + 26 weeks

INDEXER_BLOCKS_PER_CALL = 5
INDEXER_SLEEP_BETWEEN_CALL = config(
    "INDEXER_SLEEP_BETWEEN_CALL", default=5.0, cast=float
)
